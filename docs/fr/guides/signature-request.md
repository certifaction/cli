...
Les utilisateurs peuvent inviter d'autres personnes à signer des documents en leur fournissant des URL de demande de signature sécurisées, qui contiennent une clé secrète. La clé secrète déchiffrera le document avant la signature, garantissant qu'aucune tierce partie (pas même Magenta Security Sign) n'a accès au document en texte clair.

Les demandes de signature de documents Magenta Security Sign s'appuient sur l'Archive Numérique Magenta Security Sign pour stocker les documents chiffrés de bout en bout.
...
4.  Stocker le document dans l'Archive Numérique Magenta Security Sign pour récupération ultérieure.
